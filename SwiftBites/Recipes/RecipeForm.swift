...
struct RecipeForm: View {
    ...
    private var categorySection: some View {
        Section {
            Picker("Category", selection: $categoryId) {
                Text("None").tag(nil as RecipeCategory.ID?)
                ForEach(categories) { category in
                    Text(category.name).tag(category.id as RecipeCategory.ID?)
                }
            }
        }
    }
    ...
    func save() {
        do {
            ...
            if let categoryId = categoryId,
               let category = try modelContext.fetch(FetchDescriptor<RecipeCategory>(predicate: #Predicate { $0.id == categoryId })).first {
    ...
    func updateRecipe(
        ...
        guard recipes.firstIndex(where: { $0.id == id }) != nil else {
            return
        }
        let recipe = Recipe(
            id: id,
